...
class WindowAttention(nn.Module):
    def __init__(self, dim, head_dim, shifted, window_size, drop_rate=0.1):
        ...
        self.l2_norm = L2Norm()
        self.layer_norm = nn.LayerNorm(self.head_dim, elementwise_affine=False)
    ...
    def forward(self, x):
        ...
        dots = dots * rearrange(self.pos_embedding[self.relative_indices[:, :, 0], self.relative_indices[:, :, 1], :], "a b h -> 1 h 1 a b")
        if self.shifted:
...
class SwinBlock(nn.Module):
    def __init__(self, dim, head_dim, shifted, window_size, drop_rate):
        super().__init__()
        self.attention_block = PreNorm(dim, Residual(WindowAttention(dim=dim, head_dim=head_dim, shifted=shifted, window_size=window_size, drop_rate=drop_rate)))
        self.mlp_block = PreNorm(dim, Residual(FeedForward(dim=dim, hidden_dim=dim*4, drop_rate=drop_rate)))
        self.layer_norm = nn.LayerNorm(dim, elementwise_affine=False)
    ...
    def __init__(self, in_channels, dim, layer_num, downscaling_factor, head_dim, window_size, drop_rate):
        ...
        for _ in range(layer_num // 2):
            self.layer_num.append(nn.ModuleList([
                SwinBlock(dim=dim, head_dim=head_dim, shifted=False, window_size=window_size, drop_rate=drop_rate),
                SwinBlock(dim=dim, head_dim=head_dim, shifted=True, window_size=window_size, drop_rate=drop_rate),
            ]))
